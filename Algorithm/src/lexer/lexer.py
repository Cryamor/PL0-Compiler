...
class Lexer:
    ...
    def generate_token(self, match):
        ...
        if token_type == 'NEWLINE':
            ...
            raise RuntimeError(f"Unexpected character {self.code[match.start()]} at line {self.current_line} column {self.current_column}")
...
if __name__ == "__main__":
    ...
    with open('./test/token2.txt', 'w') as file:
        for token in tokens:
